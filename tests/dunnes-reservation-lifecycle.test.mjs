...
test("expires past-date Dunnes vouchers and releases timed-out reservations consistently", async () => {
  const [tidy, adminReservations, adminReviewQueue, ownerReview, state] = await Promise.all([
  ...
  for (const route of [adminReservations, adminReviewQueue, ownerReview, state]) {
  ...
  assert.match(ownerReview, /v\.expires_on >= \(now\(\) at time zone 'Europe\/Dublin'\)::date/);
});
