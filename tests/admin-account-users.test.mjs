...
test("admin Users tab manages people by account instead of anonymous hash labels", async () => {
  ...
  assert.match(panel, /full outer join profiles p on p\.auth_user_id = u\.id/);
  assert.match(panel, /u\.email/);
  assert.match(panel, /raw_app_meta_data ->> 'provider'/);
  assert.doesNotMatch(panel, /md5\(p\.id/);

  assert.match(table, /user\.email \|\| "Guest \/ 계정 미연결"/);
