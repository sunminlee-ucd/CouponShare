...
export async function GET(request: Request) {
  ...
  try {
    const result = await withSqlReconnect(async (sql) => {
      ...
      return { summaryRows, userRows, recentRows, identities };
    });

    const identityById = new Map(result.identities.map((identity) => [identity.auth_user_id, identity]));
    ...
    return Response.json(
      {
        available: true,
        summary: result.summaryRows[0] ?? EMPTY_SUMMARY,
        users: result.userRows.map(enrich),
        recent: result.recentRows.map(enrich),
      },
