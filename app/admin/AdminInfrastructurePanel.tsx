...
async function loadInfrastructureMetrics(): Promise<Metrics | null> {
  try {
    ...
    console.error("Infrastructure capacity lookup failed", error);
    return null;
  }
}
...
export default async function AdminInfrastructurePanel() {
  ...
  const metrics = await loadInfrastructureMetrics();
  if (!metrics) {
    return (
