  ...
  return (
    <>
      <button className="topbar-error-button" type="button" onClick={() => setOpen(true)}>
        <span aria-hidden="true">!</span> 오류 신고
      </button>
      {open && (
        <div className="error-report-backdrop">
          <section aria-labelledby="error-report-title" aria-modal="true" className="error-report-dialog" role="dialog">
            <header>
              <div><p>HELP US IMPROVE</p><h2 id="error-report-title">오류 신고</h2></div>
              <button type="button" onClick={close} aria-label="오류 신고 닫기">×</button>
            </header>
            {status === "sent" ? (
              ...
              <form onSubmit={submit}>
                <label>
                  ...
                  <select value={category} onChange={(event) => setCategory(event.target.value as ErrorCategory)}>
                    <option value="screen">화면·버튼</option>
                    <option value="access">로그인·접속</option>
                    <option value="coupon">쿠폰·바우처</option>
                ...
                <label>
                  ...
                  <textarea
                    ...
                    placeholder="어떤 작업에서 문제가 생겼는지 적어 주세요."
                    required
                    rows={5}
                    value={message}
                  />
                </label>
                <small>비밀번호, 전화번호 등 개인정보는 입력하지 마세요.</small>
                {notice && <p className="error-report-notice" role="alert">{notice}</p>}
                <div className="error-report-actions">
                  <button className="secondary" disabled={status === "sending"} type="button" onClick={close}>취소</button>
                  <button disabled={status === "sending" || message.trim().length < 10} type="submit">
                    {status === "sending" ? "보내는 중…" : "오류 보내기"}
                  </button>
                </div>
              </form>
            )}
